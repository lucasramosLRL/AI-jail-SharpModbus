...
public partial class AddDeviceViewModel : ObservableObject
{
    ...
    public bool IsRtu => SelectedTransport == TransportType.Rtu;
    public bool IsTcp => SelectedTransport == TransportType.Tcp;
        ...
        try
        {
            ...
            else
            {
                await foreach (var result in _scanService.ScanTcpAsync(
                    StartIp, EndIp, TcpPort, progress, token))
                {
                    var vm = new ScanResultViewModel(result);
                    await Dispatcher.UIThread.InvokeAsync(() => ScanResults.Add(vm));
                }
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            ...
            {
                var ip = string.IsNullOrWhiteSpace(DeviceIp) ? StartIp : DeviceIp;
